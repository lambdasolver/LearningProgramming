...
selection' _ _ _ []     = []
selection' _ _ _ [x]    = [[x]]
selection' i n k xs     = xs : selection' ( (i+k) `mod` (n-1) ) (n-1) k ys
    where ys = [a | a <- xs, a /= xs !! i]
...
lcSubSeq as bs = (n, reverse s)
    where   (n,s)               = last ( last lcsTab)
